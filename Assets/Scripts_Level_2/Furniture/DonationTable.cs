    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.GetComponent<Heroik>())
        {
            ...
            if(Input.GetKeyDown(KeyCode.E))
            {
                ...
                {
                    heroik.ActiveObjHands(objectOnTheTable);
                }
                else// объект есть на столе,руки заняты
                {
                    if(objectOnTheTable.name != heroik._curentTakenObjects.name)
                    {
                        Debug.Log($"Объект есть на столе: {objectOnTheTable.name}, руки заняты:{Heroik.IsBusyHands}");
                        Debug.Log($"вы пытаетесь взять объект {objectOnTheTable.name}, когда у вас в руках {heroik._curentTakenObjects.name}");
                    }
                    else
                    {
                        Debug.Log($"Объект есть на столе: {objectOnTheTable.name}, руки заняты:{Heroik.IsBusyHands}");
